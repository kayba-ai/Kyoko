...
class StorageTests(unittest.TestCase):
    def test_initialize_database_creates_schema(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(status.schema_version, 31)
            self.assertEqual(status.migration_versions, tuple(range(1, 32)))
    ...
    def test_status_json_is_machine_readable(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(decoded["schema_version"], 31)
            self.assertEqual(decoded["migration_versions"], list(range(1, 32)))
    ...
    def test_initialize_database_rejects_future_schema(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(StorageError, "database_schema_too_new:999:supported:31"):
                initialize_database(db_path)
