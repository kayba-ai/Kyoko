...
def write_consolidation_prompt_artifacts(
    *,
    db_path: Path,
    output_dir: Path,
    target: str = "generic",
    ...
    """Sibling of :func:`write_operator_prompt_artifacts` for the skillbook-consolidation
    turn — the operator is given the current active skillbook + open issues and returns
    one-or-more ``LearningProposal``\\ s that MERGE/UPDATE/DEACTIVATE duplicate skills using
    the same proposal block markers as the other turns."""
...
def build_consolidation_prompt(
    *,
    ...
    """Skillbook-consolidation turn: give the operator the current ACTIVE skillbook plus
    open issues and ask it to MERGE/UPDATE/DEACTIVATE duplicate skills, returning one-or-more
    ``LearningProposal``\\ s (same proposal block markers as the other turns)."""
    ...
    schema_display = str(schema_path) if schema_path is not None else "docs/schemas/learning-proposal.schema.json"
    target_note = _target_note(target)
    skeleton = _consolidation_proposal_skeleton(profile_id)

    active_skills = [skill for skill in list_skills(db_path, profile_id=profile_id) if skill.get("active")]
    skill_lines = [
        "- `{id}` section=`{section}` keywords={keywords} occurrences={occ}".format(
            id=skill.get("id"),
            section=skill.get("section"),
            keywords=skill.get("keywords"),
            occ=len(skill.get("occurrences") or []),
        )
    ...
    except Exception:  # noqa: BLE001 — issue listing is advisory context only
        issues = []
    open_issue_lines = [
        f"- `{issue.get('id')}` [{issue.get('status')}] {issue.get('title')}"
        for issue in issues
    ...
    return "\n".join(
        [
            "# Kyoko Skillbook Consolidation Task",
            "",
            f"Target operator: `{target}`",
            f"Evidence bundle: `{evidence_path}`",
            f"LearningProposal schema: `{schema_display}`",
            "",
            "You are acting as Kyoko's skillbook manager. The skillbook has accumulated "
            "duplicate or near-duplicate skills. Consolidate them: keep one winner skill per "
            "group of duplicates, fold the others into it, and deactivate the losers.",
            "",
            target_note,
            "",
            "## Current Active Skillbook",
            "",
            *skill_lines,
            "",
            "## Open Issues",
            ...
            *open_issue_lines,
            ...
            "## Hard Constraints",
            "",
            "- Return one-or-more proposal blocks on stdout (one per duplicate group).",
            "- Express each MERGE with existing skillbook ops only: `update` the winner with "
            "the union of keywords/occurrences, `deactivate` each loser, and `link_occurrence` "
            "to move the losers' occurrences onto the winner.",
            "- Only group skills that record the SAME learning; favor precision over recall.",
            "- Cite only skill ids and evidence ids that exist in the skillbook/evidence above.",
            "- Do not apply proposals or mutate the skillbook directly; Kyoko's gate applies them.",
            "- Keep `section` consistent within a group and on the proposal.",
            "",
            "## Required Stdout",
            "",
            "```text",
            ...
            "```",
            "",
            "The JSON above is a shape guide, not the answer. Emit one proposal block per "
            "duplicate group you decide to consolidate.",
            "",
        ]
    )


def _consolidation_proposal_skeleton(profile_id: str) -> dict[str, Any]:
    return {
        "schema_version": "kyoko.learning_proposal.v1",
        "id": "proposal_consolidate_replace_with_winner_skill_id",
        "profile_id": profile_id,
        "producer": {
            "kind": "system",
        ...
        "state": "pending",
        "section": "context",
        "title": "Consolidate duplicate context skills",
        "summary": "Merge duplicate skills into one winner.",
        "confidence": 0.6,
        "evidence_refs": [
            {
                "entity_type": "run",
                "entity_id": "replace_with_existing_run_id",
                "role": "source",
                "note": "Winner skill's source run.",
            }
        ],
        "problem": {
            "issue": "The skillbook holds duplicate entries for the same learning.",
            "severity": "low",
            "root_cause": "Repeated analyses recorded the same insight as separate skills.",
            "target": {
                "entity_type": "skill",
                "entity_id": "replace_with_winner_skill_id",
            },
        },
        "insight": "Combined guidance from the duplicate skills.",
        "proposed_changes": [
            {
                "type": "skillbook_update",
                "operation": "update",
                "skill_id": "replace_with_winner_skill_id",
                "section": "context",
                "issue": "Same reusable issue text.",
                "insight": "Combined guidance.",
            ...
            },
            {
                ...
                "skill_id": "replace_with_loser_skill_id",
                "section": "context",
                "issue": "Loser's issue text.",
                "insight": "Loser's guidance.",
                "keywords": ["loser", "keywords"],
                "occurrence_refs": [
                    {
                        "entity_type": "span",
                        "entity_id": "replace_with_existing_span_id",
        ...
        "gate_expectations": {
            ...
            "notes": "Deterministic skillbook consolidation.",
        },
