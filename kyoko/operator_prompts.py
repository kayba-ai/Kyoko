    ...
    return "\n".join(
        [
            ...
            *open_issue_lines,
            ...
            "- Each item's `body` is a short, plain-language summary for a human reviewer — what the agent did wrong and its user-facing impact, one or two sentences. It orients the reviewer before the `root_cause`; don't just repeat the title, and don't include a fix.",
...
def _issue_skeleton() -> dict[str, Any]:
    return {
        ...
        "body": "One or two plain sentences for a reviewer: what the agent did wrong and the user-facing impact (the symptom), distinct from root_cause.",
        "section": "context",
...
def _issue_update_skeleton() -> dict[str, Any]:
    ...
    return {
        ...
        "body": "Refined reviewer summary of what went wrong and its impact (or keep the existing one).",
        "section": "context",
